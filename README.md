...
查询数据的执行逻辑是“查缓存->查数据库”，由于缓存已经挂了，因此每次“查缓存”这一步都会报连接超时的错误，因为缓存层已经不能提供服务，那么每次查询都查询一遍缓存已经没有意义。解决办法是如果缓存层在一定的访问次数内都连接不上，则切断缓存层的查询入口，不走缓存查询逻辑，并开启一个定时任务，定时访问缓存，探测缓存层是否已经恢复，如果已经恢复，则继续走查询缓存逻辑。
...
如果某些key不存在，想要过滤掉这些数据的访问（避免缓存穿透），要怎么办？

如果查询到一些key不存在，则可以设置一个默认值到缓存中，这涉及到具体的业务逻辑处理，可以根据具体业务在查询时进行设置返回；也可以使用类似布隆过滤器等方式，对这些不存在的key做前置过滤，避免直接查询缓存穿透到数据库。

那么这些前置过滤要加在哪里？每个查询的地方都布隆过滤先过滤一遍？
